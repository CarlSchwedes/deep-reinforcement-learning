...
from model import QNetwork, DuelingQNetwork, DistributionalDuelingQNetwork
    ...
    def act(self, state, eps=0.):
        ...
        with torch.no_grad():
            if not self.use_distributional_rl:
                ...
                support_expanded = self.support.unsqueeze(0).unsqueeze(0)  # Shape: (1, 1, 51)
                q_values = (probs * support_expanded).sum(dim=-1) # Shape: (1, action_size)
