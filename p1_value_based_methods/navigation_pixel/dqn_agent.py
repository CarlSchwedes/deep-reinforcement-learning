...
class Agent():
    ...
    def step(self, state, action, reward, next_state, passed_done, unity_done):
        """Execute one step of the DQN learning process.
        
        Params
        ======
            passed_done (bool): Masked done flag (False on timeouts) for Bellman target.
            unity_done (bool): Raw environment done flag to control episode termination.
        """
        # 1. Add to n-step buffer using the passed_done flag for bootstrapping calculations
        self.n_step_buffer.append((state, action, reward, next_state, passed_done))
        
        # 2. Process experiences if the buffer is full OR if the episode physically ended
        if len(self.n_step_buffer) == self.n_steps or unity_done:
            
            # Loop through and process all remaining items if the episode is physically over
            while len(self.n_step_buffer) > 0:
                # Compute n-step discounted return
                n_step_return = 0.0
                for i, (s, a, r, ns, d) in enumerate(self.n_step_buffer):
                    n_step_return += r * (GAMMA ** i)
                    if d:  # If a real termination (passed_done=True) happens mid-buffer, stop discounting
                        break
                
                # Extract starting state and final transition state
                first_state, first_action, _, _, _ = self.n_step_buffer[0]
                
                # Identify the look-ahead horizon point
                # If a real termination happened inside the buffer, look up to that termination point
                term_index = next((idx for idx, tuple_item in enumerate(self.n_step_buffer) if tuple_item[4]), -1)
                if term_index != -1:
                    last_state, _, _, last_next_state, last_done = self.n_step_buffer[term_index]
                else:
                    last_state, _, _, last_next_state, last_done = self.n_step_buffer[-1]

                # --- RAM OPTIMIZATION: Convert frames to uint8 for storage ---
                first_state_uint8 = (first_state * 255).astype(np.uint8)
                last_next_state_uint8 = (last_next_state * 255).astype(np.uint8)
                
                # Push the compressed multi-step trajectory to the replay memory
                self.memory.add(first_state_uint8, first_action, n_step_return, last_next_state_uint8, last_done)
                
                # If it's a standard mid-game update, just pop the oldest item and continue normal operation
                if not unity_done:
                    self.n_step_buffer.popleft()
                    break
                else:
                    # If unity_done is True, we drain the entire buffer step-by-step 
                    # so no experience is lost at the end of the episode
                    self.n_step_buffer.popleft()
            
            # 3. Clean wrap-up: Force clear the buffer at the end of the match to prevent cross-contamination
            if unity_done:
                self.n_step_buffer.clear()
        
        # ======================== LEARN SUBROUTINE (Unchanged) ========================
    ...
    def learn(self, experiences, gamma, indices=None, is_weights=None):
        ...
        Params
        ======
            experiences (Tuple[torch.Tensor]): tuple of (s, a, r, s', passed_done, unity_done) tuples 
        ...
        """
        states, actions, rewards, next_states, passed_done = experiences
        ...
        Q_targets = rewards + (gamma * Q_targets_next * (1 - passed_done))
...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed):
        ...
        self.batch_size = batch_size
        self.experience = namedtuple("Experience", field_names=["state", "action", "reward", "next_state", "passed_done"])
        self.seed = random.seed(seed)
    
    def add(self, state, action, reward, next_state, passed_done):
        """Add a new experience to memory."""
        e = self.experience(state, action, reward, next_state, passed_done)
    ...
    def sample(self):
        ...
        passed_done = torch.from_numpy(np.vstack([e.passed_done for e in experiences if e is not None]).astype(np.uint8)).float().to(device)
  
        return (states, actions, rewards, next_states, passed_done)
